...
public class Grid<ObjectType>
{
    ...
    public Grid(int width, int height, float cellsize, Vector3 origin, Func<Grid<ObjectType>, int, int, ObjectType> Object)
    {
        this.width = width;
        this.height = height;
    ...
    public AstarAlgorithm(int[,] arr)
    {
        ...
        for (int i = 0; i < arr.GetLength(0); i++)
            for (int j = 0; j < arr.GetLength(1); j++)
                if (arr[i, j] == 0)
                    grid.GetObject(i, j).isWalkable = false;
    }
